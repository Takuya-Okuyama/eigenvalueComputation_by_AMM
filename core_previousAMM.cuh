...
template <uint64_t nthreads_per_block,
          uint64_t NUM_UNROLL>
__global__ __launch_bounds__(nthreads_per_block) void previousAMM_kernel1(
    const uint32_t m,
    ...
    const float *__restrict__ d_weight)
{
  ...
  float4 regw, regA[8], regx[2], regx_memo[2];
...
#pragma unroll
  ...
  if (threadIdx.x == 0)
  {
    vstore(tmp + 4 * blockIdx.x, ret * regw);
  }

  uint64_t i = blockIdx.x * blockDim.x + threadIdx.x;
  for (; i < m / 4; i += blockDim.x * gridDim.x)
  {
    reinterpret_cast<float4 *>(y)[i] = reinterpret_cast<const float4 *>(y_memo)[i];
  }
}
...
template <uint64_t nthreads_per_block,
          ...
          uint64_t NUM_UNROLL>
__global__ __launch_bounds__(nthreads_per_block) void previousAMM_kernel2(
    const uint64_t m,
    const uint64_t k,
    const float *__restrict__ A,
    const float *__restrict__ x,
...
#pragma unroll
    ...
    resY += vecA[0] * sx[NUM_UNROLL - 2] + vecA[1] * sx[NUM_UNROLL - 1];
  }
  __syncthreads();
